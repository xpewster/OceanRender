...
{
	bool usingKdTree = traceUI->kdSwitch();
	double tmin      = 0.0;
	double tmax      = 0.0;
	if (usingKdTree) {
		if (!hasBoundingBoxCapability() ||
		    !localBounds.intersect(r, tmin, tmax))
			return false;
		return kdtree->findIntersection(max(0.0, tmin), tmax, r, i);
	} else {
		bool have_one = false;
		for (auto face : faces) {
			isect cur;
...
bool TrimeshFace::intersectLocal(ray& r, isect& i) const
{
	...
	if (alpha >= 0 && beta >= 0 && gamma >= 0 && alpha+beta+gamma > 1.0-RAY_EPSILON && alpha+beta+gamma < 1.0+RAY_EPSILON){
		i.setObject(this);
		// if (std::isnan(t)){
		// 	std::cout << "BAD T!\n" << t << std::endl << dist << std::endl << glm::to_string(normal) << std::endl << glm::to_string(r.getPosition()) << std::endl << _dot << std::endl;
		// }
		i.setT(t);
...
{
	kdtree.reset(kdtree->buildTree(faces, ComputeLocalBoundingBox(), 0));
}
